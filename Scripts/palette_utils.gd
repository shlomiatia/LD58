...
static func get_adjacent_colors(palette_index: int, start_index: int, count: int) -> Array:
	var colors = []
	for i in range(count):
		colors.append(COLOR_PALETTE[palette_index][start_index + i])
	return colors

static func select_random_colors_from_palettes(palette_indices: Array, count: int) -> Array:
	var palette_index = palette_indices[randi() % palette_indices.size()]
	var start_index = randi_range(0, COLOR_PALETTE[palette_index].size() - count)
	return get_adjacent_colors(palette_index, start_index, count)
